...
class CRM_Xdedupe_MergeJob {
   ...
   *
   * @param string $dedupe_run_id dedupe run
   * @param array<string, mixed> $params parameters for API Xdedupe.merge
   * @param string|NULL $onEndUrl URL to return to after the runner is finished. Default is the control room
   */
  public static function launchMergeRunner(string $dedupe_run_id, array $params, ?string $onEndUrl = NULL): void {
    // create a queue
   ...
   * @param int|NULL $offset offset within the dedupe run
   * @param array|NULL $tuples tuples [main_contact_id => other contact IDs]
   */
  protected function __construct(
    string $mode,
    string $dedupe_run_id,
    array $params,
    ?int $offset = NULL,
    ?array $tuples = NULL
  ) {
   ...
   * Run the taks
   *
   * @param CRM_Queue_TaskContext $context CRM_Queue_TaskContext
   *
   * @return bool       success
   * @throws Exception  shouldn't...
   */
  public function run(CRM_Queue_TaskContext $context): bool {
    switch ($this->mode) {
      case 'merge':
        ...
        foreach ($this->tuples as $main_contact_id => $other_contact_ids) {
          ...
          $merger->multiMerge((int) $main_contact_id, $other_contact_ids);
      ...
      case 'summary':
        ...
        $tuples_merged = CRM_Core_DAO::singleValueQuery(
          "SELECT COUNT(merged_count) FROM `{$table_name}` WHERE merged_count > 0;"
        );
        // don't count the main contacts
        $contact_count = $dedupe_run->getContactCount() - $tuple_count;
        $contacts_merged = CRM_Core_DAO::singleValueQuery("SELECT SUM(merged_count) FROM `{$table_name}`;");
        CRM_Core_Session::setStatus(
          E::ts(
            'Merged %1 of %2 tuples, %3 of %4 contacts.',
            [
              1 => $tuples_merged,
              2 => $tuple_count,
              3 => $contacts_merged,
        ...
        if (($this->params['config_id'] ?? 0) !== 0) {
          ...
          // todo: get? from where?
          $stats['failed'] = [];
          $stats['merger_runtime'] = time() - strtotime($stats['last_run']);
