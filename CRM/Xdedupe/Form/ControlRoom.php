...
class CRM_Xdedupe_Form_ControlRoom extends CRM_Core_Form {
  ...
  public static function getTupleRowsAJAX() {
    ...
    if (!empty($tuples)) {
      ...
      foreach ($tuples as $main_contact_id => $contact_ids) {
        ...
        // phpcs:disable Squiz.PHP.CommentedOutCode.Found,Generic.Files.LineLength.TooLong
        // add compare link
  ...
  // phpcs:enable
    ...
    $this->assign('config_hidden', FALSE);
    $this->assign('config_showing', FALSE);
    $this->assign('result_count', 0);
