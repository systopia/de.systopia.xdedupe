...
class CRM_Xdedupe_Page_Manager extends CRM_Core_Page {
  ...
  protected function renderConfiguration($configuration) {
    $data = array(
      ...
      'category'       => $configuration->getAttribute('category'),
      'schedule'       => $this->renderSchedule($configuration->getAttribute('scheduled')),
      'last_executed'  => $this->renderDate($configuration->getAttribute('last_execution')),
      'last_runtime'   => $this->renderRuntime($configuration->getAttribute('last_runtime')),
      'parallel_exec'  => $configuration->getAttribute('parallel_exec'),
      'next_execution' => 'TODO', //$this->renderDate($configuration->getNextExecutionTime()),
