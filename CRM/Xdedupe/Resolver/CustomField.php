...
class CRM_Xdedupe_Resolver_CustomField extends CRM_Xdedupe_Resolver {
  ...
  public function getName(): string {
    ...
    $field_name = civicrm_api4('CustomField', 'get', [
      'select' => ['label', 'custom_group_id'],
      'where' => [['id', '=', $this->custom_field_id]],
    ]);
    $group_name = civicrm_api4('CustomGroup', 'get', [
      'select' => ['title'],
      'where' => [['id', '=', $field_name[0]['custom_group_id']]],
    ]);
  ...
  public function resolve($main_contact_id, $other_contact_ids): bool {
    ...
    if ($resolved_contact_value) {
      $this->addMergeDetail(
        E::ts(
          "Picked value(s) '$resolved_contact_value' from contact [%1]",
          [1 => $merged_value_origin_contact_id]
        )
      );
