...
abstract class CRM_Xdedupe_Resolver_OptionValueAttribute extends CRM_Xdedupe_Resolver_SimpleAttribute {
  ...
  protected string $option_group_name;

  public function __construct(?CRM_Xdedupe_Merge $merge, string $attribute_name, string $option_group_name) {
  ...
  public function resolve($main_contact_id, $other_contact_ids): bool {
    ...
    foreach ($values as $value) {
      $weight = $value2weight[$value] ?? PHP_INT_MAX;
   ...
   */
  protected function getValueToWeight(): ?array {
  ...
  /**
   * Get the attribute used as the value in CiviCRM. 'value' is default, obviously,
   * but e.g. preferred_language stores 'name'
   *
